<template>
  ...
  export default {
      setup () {
        ...
        const { actualizarObjetivosKpis } = useKpi
        ...
        const abrir = async(objetivo) => {
          ...
          sucursalSeleccionada.value = opcionesSucursales.value.find(sucursal => sucursal.value.abreviacion === objetivo.claveSucursal)
          objKpi.value = {...objetivo}
        ...
        const guardarKpis = async() => {
          ...
          await actualizarObjetivosKpis(objKpi.value)
          modalObjetivo.value = false
        }
