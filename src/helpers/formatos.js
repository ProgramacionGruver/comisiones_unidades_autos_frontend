...
export const formatearGratificaciones = (row) => {
  if (row.tipoGratificacion === 'vacaciones' || row.tipoGratificacion === 'capacitacion') {
    return row.monto + ' días';
  } else {
    return row.monto?.toLocaleString('es-MX', { style: 'currency', currency: 'MXN', maximumFractionDigits: 2 });
  }
}
...
export const formatearNombrePuesto = (nombre) => {
  ...
  palabrasAEliminar.forEach(palabra => {
    nombreFormateado = nombreFormateado.replace(new RegExp('\\b' + palabra + '\\b', 'g'), '')
  })
