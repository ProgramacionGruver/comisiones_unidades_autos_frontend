        ...
        <div class="q-px-md row items-start q-gutter-md row justify-center">
          <q-card class="my-card">
            <q-card-section class="inline justify-center">
                <div class="text-h5 text-center text-weight-bolder">{{facturaSeleccionada[0]?.factura}}</div>
                <div class="text-h6 text-center text-weight-medium">({{facturaSeleccionada[0]?.tipo_venta}})</div>
              </q-card-section>
            <q-card-section class="row justify-around q-px-lg">
                <div class="text-h5 text-center">Fecha factura: <span class='text-weight-medium'> {{ formatearFecha(facturaSeleccionada[0]?.fecha_facturacion) }} </span></div>
                <div class="text-h5 text-center">Utilidad Bruta: <span class='text-weight-medium'> {{formatearMonto(facturaSeleccionada[0]?.utilidad)}}</span></div>
                 <div class="text-h5 text-center">Gasto Financiero: <span class='text-weight-medium'> {{formatearMonto(facturaSeleccionada[0]?.gastoFinanciero)}}</span></div>
                 <div class="text-h5 text-center">Utilidad: <span class='text-weight-medium'> {{formatearMonto(facturaSeleccionada[0]?.utilidadCalculada)}}</span></div>
              </q-card-section>
            <q-card-section class="inline justify-center q-px-md">
                <div class="text-center">
                  <q-expansion-item  class="text-h5 text-weight-bolder"
                    v-model="expanded"
                    label="Desglose"
                  >
...
import { formatearCapitalCase, formatearMonto, formatearPorcentajeDecimal } from 'src/helpers/formatos'
...
export default {
  ...
  setup () {
    ...
    const {facturas, facturaSeleccionada,facturasFiltrada, valoresUnicosSucursal ,cargando, anioSeleccionado, mesSeleccionado, quincenaSeleccionada} = storeToRefs(useFacturas)
    ...
    const columns = [
      ...
      },
      {
        ...
        field:row => row.gastoFinanciero ? formatearMonto(row.gastoFinanciero) : formatearMonto(0),
        align: 'left',
        sortable: true
      },
      {
        ...
        label: 'Utilidad',
        field: row => row.utilidadCalculada ? formatearMonto(row.utilidadCalculada) : formatearMonto(0),
        align: 'left',
        sortable: true
      },
      {
        name: 'porcentaje',
        label: 'Porcentaje',
        field: row => row.porcentaje ? formatearPorcentajeDecimal(row.porcentaje) : formatearPorcentajeDecimal(0),
        align: 'left',
        sortable: true
      },
    ]
    ...
    const columnsDetalles = [
      ...
      },
      {
        name: 'importe_venta',
        label: 'Importe Venta',
        field: row => row.importe_venta ? formatearMonto(row.importe_venta) : formatearMonto(0),
        align: 'left',
        sortable: true
      },
      {
      ...
      },
      {
        name: 'importe_total',
        label: 'Importe Total',
        field: row => row.importe_total ? formatearMonto(row.importe_total) : formatearMonto(0),
        align: 'left',
        sortable: true
      },
      {
      ...
      },
      {
        name: 'desc_cxc',
        label: 'Descuentos',
        field: row => row.desc_cxc ? formatearMonto(row.desc_cxc) : formatearMonto(0),
        align: 'left',
        sortable: true
      },
      {
